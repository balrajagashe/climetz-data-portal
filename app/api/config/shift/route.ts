...
export async function GET(req: NextRequest) {
  ...
  const doc = await ShiftConfig.findOne({ locationRef: location, effective_from: { $lte: at } }).sort({ effective_from: -1 }).lean();
...
export async function PUT(req: NextRequest) {
  ...
  const doc = await ShiftConfig.create(body);
